<template>
  <div class="song-start">
    ...
    <!-- 返回上一页按钮 -->
    <div class="start-fanhui" @click="TB">
    ...
    <div class="start-prev" @click="getIndex1">
      <img src="../assets/img/play/last_night.png" alt="" />
    </div>
    <div class="start-next" @click="getIndex">
      <img src="../assets/img/play/next_night.png" alt="" />
    </div>
      ...
      <div class="popup-content">
        ...
        >
          ...
          <div class="popup-content-item-right">
            <div class="popup-item-songName" :class="{ selected1: id == d.id }">
              {{ d.name }}
            </div>
            <div class="popup-singer-item">
              <div
                class="popup-item-singerName"
                v-for="n in d.ar"
                :key="n.id"
                :class="{ selected1: id == d.id }"
              >
                {{ n.name }}
              </div>
            </div>
    ...
    >
      <div class="popup-time">
        ...
        <div class="popup-time-select">
          <div :class="{ active1: active == 0 }" @click="changeActive(0)">
            关闭
          </div>
          <div :class="{ active1: active == 1 }" @click="changeActive(1)">
            10
          </div>
          <div :class="{ active1: active == 2 }" @click="changeActive(2)">
            20
          </div>
          <div :class="{ active1: active == 3 }" @click="changeActive(3)">
            30
          </div>
          <div :class="{ active1: active == 4 }" @click="changeActive(4)">
            60
          </div>
          <div :class="{ active1: active == 5 }" @click="changeActive(5)">
            90
          </div>
        </div>
      </div>
    ...
    ></div>
            ...
            <div class="detail-list"  @click="show1 = false">
              <div
                class="detail-bottom"
...
export default {
  ...
  methods: {
    ...
    //不喜歡
    noLike() {
      this.detailArr1.map((item, index) => {
        if (this.id == item.id) {
          this.index1 = index;
        }
      });
      if (this.index1 == this.detailArr1.length - 1) {
        this.id = this.detailArr1[this.index1 - 3].id;
      } else {
        this.id = this.detailArr1[this.index1 + 1].id;
        this.detailArr1.filter((item, index) => index == this.index1);
        this.LocalStorage();
        this.morePopup = false;
        Toast("操作成功,将减少此类音乐推荐");
    ...
    addLove1(id) {
      ...
      if (this.getLoveArr.indexOf(id) == -1) {
        ...
        this.getLoveArr = this.getLoveArr.filter(
          (item, index) => index !== this.getLoveArr.indexOf(id)
        );
        this.LocalStorage();
    ...
    getLocalStorageData1() {
      if (!localStorage.Arr) {
        ...
        console.log("adfgh", this.getLoveArr);
      }
    },
    //  分享
    ...
    changePlayState() {
      this.$store.commit("setAudioPlayState", true);
    },
    getToDetailDate() {
  ...
  watch: {
    ...
    getPlayId(newId, oldId) {
      console.log("a", this.audioPlayState);
      if (newId !== oldId) {
        this.getLyricData();
...
.popup-time {
  ...
  .popup-time-select {
    ...
    margin-top: 30px;
    padding: 3px;
    background-color: rgb(245, 244, 244, 0.1);
    div {
      width: 16.62%;
      text-align: center;
      padding: 20px 0;
